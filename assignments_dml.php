...
function assignments_insert(){
	...
	if(function_exists('assignments_after_insert')){
		$res = sql("select * from `assignments` where `Id`='" . makeSafe($recID) . "' limit 1", $eo);
...
function assignments_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0){
	...
	$combo_ProjectId->HTML = '<span id="ProjectId-container' . $rnd1 . '"></span><input type="hidden" name="ProjectId" id="ProjectId' . $rnd1 . '" value="' . htmlspecialchars($combo_ProjectId->SelectedData, ENT_QUOTES, 'UTF-8') . '">';
	$combo_ProjectId->MatchText = '<span id="ProjectId-container-readonly' . $rnd1 . '"></span><input type="hidden" name="ProjectId" id="ProjectId' . $rnd1 . '" value="' . htmlspecialchars($combo_ProjectId->SelectedData, ENT_QUOTES, 'UTF-8') . '">';
	$combo_ResourceId->HTML = '<span id="ResourceId-container' . $rnd1 . '"></span><input type="hidden" name="ResourceId" id="ResourceId' . $rnd1 . '" value="' . htmlspecialchars($combo_ResourceId->SelectedData, ENT_QUOTES, 'UTF-8') . '">';
	$combo_ResourceId->MatchText = '<span id="ResourceId-container-readonly' . $rnd1 . '"></span><input type="hidden" name="ResourceId" id="ResourceId' . $rnd1 . '" value="' . htmlspecialchars($combo_ResourceId->SelectedData, ENT_QUOTES, 'UTF-8') . '">';
	...
	<script>
		...
		var current_ProjectId__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['ProjectId'] : $filterer_ProjectId); ?>"};
		var current_ResourceId__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['ResourceId'] : $filterer_ResourceId); ?>"};
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			jQuery("#ProjectId-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: current_ProjectId__RAND__.value, t: 'assignments', f: 'ProjectId' }
					}).done(function(resp){
						c({
							id: resp.results[0].id,
							text: resp.results[0].text
						});
						jQuery('[name="ProjectId"]').val(resp.results[0].id);
						jQuery('[id=ProjectId-container-readonly__RAND__]').html('<span id="ProjectId-match-text">' + resp.results[0].text + '</span>');
				...
				current_ProjectId__RAND__.value = e.added.id;
				current_ProjectId__RAND__.text = e.added.text;
				jQuery('[name="ProjectId"]').val(e.added.id);
		...
		<?php }else{ ?>
			...
			jQuery.ajax({
				...
				jQuery('[id=ProjectId-container__RAND__], [id=ProjectId-container-readonly__RAND__]').html('<span id="ProjectId-match-text">' + resp.results[0].text + '</span>');
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			jQuery("#ResourceId-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c){
					jQuery.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: current_ResourceId__RAND__.value, t: 'assignments', f: 'ResourceId' }
					}).done(function(resp){
						c({
							id: resp.results[0].id,
							text: resp.results[0].text
						});
						jQuery('[name="ResourceId"]').val(resp.results[0].id);
						jQuery('[id=ResourceId-container-readonly__RAND__]').html('<span id="ResourceId-match-text">' + resp.results[0].text + '</span>');
				...
				current_ResourceId__RAND__.value = e.added.id;
				current_ResourceId__RAND__.text = e.added.text;
				jQuery('[name="ResourceId"]').val(e.added.id);
		...
		<?php }else{ ?>
			...
			jQuery.ajax({
				...
				jQuery('[id=ResourceId-container__RAND__], [id=ResourceId-container-readonly__RAND__]').html('<span id="ResourceId-match-text">' + resp.results[0].text + '</span>');
	...
	if($dvprint){
		$templateCode = @file_get_contents('./templates/assignments_templateDVP.html');
	}else{
		$templateCode = @file_get_contents('./templates/assignments_templateDV.html');
	}
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode=str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return assignments_validateData();"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(ProjectId)%%>', htmlspecialchars($row['ProjectId'], ENT_QUOTES, 'UTF-8'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(ProjectId)%%>', urlencode($urow['ProjectId']), $templateCode);
		$templateCode=str_replace('<%%VALUE(ResourceId)%%>', htmlspecialchars($row['ResourceId'], ENT_QUOTES, 'UTF-8'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(ResourceId)%%>', urlencode($urow['ResourceId']), $templateCode);
		$templateCode=str_replace('<%%VALUE(Commitment)%%>', htmlspecialchars($row['Commitment'], ENT_QUOTES, 'UTF-8'), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(Commitment)%%>', urlencode($urow['Commitment']), $templateCode);
		$templateCode=str_replace('<%%VALUE(StartDate)%%>', @date('d/m/Y', @strtotime(htmlspecialchars($row['StartDate'], ENT_QUOTES, 'UTF-8'))), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(StartDate)%%>', urlencode(@date('d/m/Y', @strtotime(htmlspecialchars($urow['StartDate'], ENT_QUOTES, 'UTF-8')))), $templateCode);
		$templateCode=str_replace('<%%VALUE(EndDate)%%>', @date('d/m/Y', @strtotime(htmlspecialchars($row['EndDate'], ENT_QUOTES, 'UTF-8'))), $templateCode);
		$templateCode=str_replace('<%%URLVALUE(EndDate)%%>', urlencode(@date('d/m/Y', @strtotime(htmlspecialchars($urow['EndDate'], ENT_QUOTES, 'UTF-8')))), $templateCode);
	}else{
	...
	if($dvprint){
		...
		$templateCode.="\t\t\t'assignments_autofill.php?rnd1=$rnd1&mfk=ProjectId&id=' + encodeURIComponent(current_ProjectId{$rnd1}.value),\n";
		$templateCode.="\t\t\t{encoding: 'UTF-8', method: 'get', onCreate: function(){ \$('ProjectId$rnd1').disable(); \$('ProjectIdLoading').innerHTML='<img src=loading.gif align=top>'; }, onComplete: function(){".(($arrPerm[1] || (($arrPerm[3]==1 && $ownerMemberID==getLoggedMemberID()) || ($arrPerm[3]==2 && $ownerGroupID==getLoggedGroupID()) || $arrPerm[3]==3)) ? "\$('ProjectId$rnd1').enable(); " : "\$('ProjectId$rnd1').disable(); ")."\$('ProjectIdLoading').innerHTML='';}}\n";
	}
	$templateCode.="\t\t);\n";
	$templateCode.="\t};\n";
	if(!$dvprint) $templateCode.="\tif(\$('ProjectId_caption') != undefined) \$('ProjectId_caption').onchange=ProjectId_update_autofills$rnd1;\n";
