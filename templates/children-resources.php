<script>
	resourcesGetChildrenRecordsList = function(command){
		var param = {
		...
		switch(command.Verb){
			...
			case 'new': /* new record */
				...
				Modalbox.show('<iframe src="' + url + '" seamless="seamless" width="' + (mbWidth - 40) + '" height="' + (mbHeight - 50) + '" sandbox="allow-forms allow-scripts allow-same-origin"></iframe>', {
					...
					afterHide: function(){ resourcesGetChildrenRecordsList({ Verb: 'reload' }); },
					width: mbWidth,
					height: mbHeight,
...
<table cellpadding="0" border="0" cellspacing="1" width="100%">
	<tr>
		<td class="toolbar" colspan="<?php echo (count($config['display-fields']) + ($config['open-detail-view-on-click'] ? 1 : 0)); ?>">
			<?php if($config['display-add-new']){ ?><div title="<?php echo addslashes($Translation['Add New']); ?>" onclick="resourcesGetChildrenRecordsList({ Verb: 'new' });"><img src="addNew.gif" /></div><?php } ?>
			<?php if($config['display-refresh']){ ?><div onclick="resourcesGetChildrenRecordsList({ Verb: 'reload' });"><img src="arrow_refresh.png" /></div><?php } ?>
		</td>
	</tr>
	<tr>
		<?php if(is_array($config['display-fields'])) foreach($config['display-fields'] as $fieldIndex => $fieldLabel){ ?>
			...
			<?php if($config['sortable-fields'][$fieldIndex]){ ?>
				onclick="resourcesGetChildrenRecordsList({
					Verb: 'sort', 
	...
	<tr class="colorize">
		...
		<?php if($config['open-detail-view-on-click']){ ?>
			<td onclick="resourcesGetChildrenRecordsList({ Verb: 'open', ChildID: '<?php echo addslashes($record[$config['child-primary-key-index']]); ?>'});" class="<?php echo ($i ? 'TableBody' : 'TableBodySelected'); ?> view-on-click"><img src="view.gif" /></td>
		<?php } ?>
	</tr>
	<?php } ?>
	...
	<tr>
		<td class="TableFooter" colspan="<?php echo (count($config['display-fields']) + ($config['open-detail-view-on-click'] ? 1 : 0)); ?>">
			<div style="float: left;">
				<?php if($totalMatches){ ?>
					<img align="left" src="deselect.gif" style="cursor: pointer; margin-top: 3px;" onclick="resourcesGetChildrenRecordsList({ Verb: 'page', Page: 'previous' });" />
					<img align="left" src="nextPage.gif" style="cursor: pointer; margin-top: 3px;" onclick="resourcesGetChildrenRecordsList({ Verb: 'page', Page: 'next' });" />
